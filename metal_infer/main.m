...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        int do_verify = 0;
        int use_fast = 0;
        // Resolve default model path: FLASH_MOE_MODEL env var, or HuggingFace cache
        static char default_model_path[1024];
        const char *env_model = getenv("FLASH_MOE_MODEL");
        if (env_model && env_model[0]) {
            snprintf(default_model_path, sizeof(default_model_path), "%s", env_model);
        } else {
